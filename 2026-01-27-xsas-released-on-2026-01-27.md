---
layout: post
title: "XSAs released on 2026-01-27"
categories: security
---
...
- [XSA-477](https://xenbits.xen.org/xsa/advisory-477.html)
  - This XSA affects only HVMs with shadow paging and tracing enabled. In Qubes OS, shadow paging and
tracing are disabled at build time.
- [XSA-478](https://xenbits.xen.org/xsa/advisory-478.html)
  - This XSA affects only XAPI, which is an alternative toolstack. Qubes OS uses libxl instead of XAPI.
- [XSA-479](https://xenbits.xen.org/xsa/advisory-479.html)
  - This XSA affects only in-VM isolation, which Qubes OS does not rely on for security. We will still provide the fix for this issue at a later date, but it will not be accompanied by a [Qubes security bulletin (QSB)](/security/qsb/).
